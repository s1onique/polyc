...
## PUSH STATUS (separate from compiler work)

The INTOPS01 commit (`da613906...`) is **not pushed**. The local
main and `origin/main` are SHA-divergent (382 vs 374 commits past
their common base). Cherry-equivalence analysis (`git rev-list
--cherry-mark`) reduces that to:

```
LOCAL_PATCH_UNIQUE  =  13  (8 substantive PolyC Factory commits
                              + 5 merge SHA collisions)
REMOTE_PATCH_UNIQUE =   5  (5 merge SHA collisions)
CHERRY_EQUIVALENT   = 738  (same content, different SHAs from
                              parallel history rewrites)
```

The 8 substantive local commits are PARITY01's 5-chain + 2 ROADMAP
edits + INTOPS01 CLOSE — none of these exist on remote. Everything
else is Git plumbing noise from prior history rewrites.

Both tips are archived before any reconciliation work:
```
archive/local-main-before-reconcile    = adb202c
archive/origin-main-before-reconcile   = 429b804
archive-local-main-2026-09             = adb202c
archive-origin-main-2026-09            = 429b804
```

The push decision is **outside INTOPS01's scope** (F15: never
self-authorize scope expansion). It belongs to a separate bounded
Factory-v2 ACT (`ACT-POLYC-FACTORY-HISTORY-RECONCILE01` or
equivalent). Three legitimate append-only patterns are documented
in `evidence/llvm-intops01/closure/remote-topology/HALT-PUSH-DECISION-REQUIRED.md`;
none has been chosen or executed yet.
