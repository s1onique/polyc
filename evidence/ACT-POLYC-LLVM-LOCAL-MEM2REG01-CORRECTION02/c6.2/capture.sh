...
EVID=$REPO/evidence/ACT-POLYC-LLVM-LOCAL-MEM2REG01-CORRECTION02/c6.2
mkdir -p "$EVID"
...
# self-test call it.
validate_row() {
    pre_rc="$1"; post_rc="$2"; verify_rc="$3"
    stores_per_alloca="$4"; synth_store_count="$5"
...
capture_one() {
    ...
    TOTAL=$((TOTAL + 1))
    pre_ll=/tmp/c6.2/${tag}.pre.ll
    post_ll=/tmp/c6.2/${tag}.post.ll

    if HCC_NO_MEM2REG=1 "$HCC" --emit-llvm $HCC_INSTALL_ARG "$path" \
        -o "$pre_ll" >/tmp/c6.2/${tag}.pre.stderr 2>&1; then
        pre_rc=0
    else
        pre_rc=$?
    fi
    ...
    if "$HCC" --emit-llvm $HCC_INSTALL_ARG "$path" \
        -o "$post_ll" >/tmp/c6.2/${tag}.post.stderr 2>&1; then
        post_rc=0
    else
        post_rc=$?
    fi
    ...
    if $OPT -passes=verify "$post_ll" -disable-output \
        >/tmp/c6.2/${tag}.post.verify.stdout 2>/tmp/c6.2/${tag}.post.verify.stderr; then
        verify_rc=0
    else
        verify_rc=$?
    fi

    if $OPT -passes=mem2reg "$post_ll" -S -o /tmp/c6.2/${tag}.post.idem.ll \
        >/tmp/c6.2/${tag}.post.idem.stdout 2>/tmp/c6.2/${tag}.post.idem.stderr; then
        idem_rc=0
    else
        idem_rc=$?
    fi
    ...
    if [ "$idem_rc" -eq 0 ]; then
        post_norm=/tmp/c6.2/${tag}.post.norm.ll
        idem_norm=/tmp/c6.2/${tag}.post.idem.norm.ll
        sed -E "s|source_filename = \"[^\"]*\"|source_filename = \"REDACTED\"|; s|^; ModuleID = '[^']*'|; ModuleID = 'REDACTED'|" \
            "$post_ll" > "$post_norm"
        sed -E "s|source_filename = \"[^\"]*\"|source_filename = \"REDACTED\"|; s|^; ModuleID = '[^']*'|; ModuleID = 'REDACTED'|" \
            /tmp/c6.2/${tag}.post.idem.ll > "$idem_norm"
    ...
    if [ "$idem_rc" -eq 0 ]; then
        /bin/cp /tmp/c6.2/${tag}.post.idem.ll "$EVID/${name}.post-mem2reg.idempotent.ll"
    fi
    ...
    row=$(validate_row "$pre_rc" "$post_rc" "$verify_rc" \
...
if [ "${C6_2_SELF_TEST:-0}" = "1" ]; then
    ...
    fail_result=$(validate_row 1 0 0 OK 0)
    fail_status=${fail_result%%|*}
    pass_result=$(validate_row 0 0 0 OK 0)
...
if [ "${C6_2_NEG_TEST:-0}" = "1" ]; then
    ...
    FIXTURES_FILE=/tmp/c6.2/fixtures.txt
...
# single mechanical truth-channel for c6.2 evidence.
if [ "$FAIL" -eq 0 ]; then
    exit 0
else
    exit 1
