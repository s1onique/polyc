...
const LLVMBackendCapabilityDef kLLVMBackendCapability[] = {
    ...
    { IR_LOAD_DEREF, LLVMBC_SHAPE_DEPENDENT, NULL, "ACT-POLYC-LLVM-MEMORY01: SHAPE_DEPENDENT. Supported when (a) r1 (addr) is an IR_VAL_PARAM with type IR_TYPE_PTR mapped to LLVM opaque ptr, and (b) dst->type is IR_TYPE_I64. All other shapes rejected with LLVM_BACKEND_UNSUPPORTED_POINTER (or LLVM_BACKEND_UNSUPPORTED_TYPE if dst->type is not the proven I64 access type)." },
